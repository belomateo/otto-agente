# Ficha del negocio — Mr Otto / Otto Su Misura

Respondida por Mr Otto (8/9/2026) y completada con Mateo (12/9/2026). Es DATO:
se carga en tablas y fragmentos, no en el prompt. Lo marcado ➜ son respuestas
posteriores de Mateo que completan o corrigen la ficha original.

## Identidad del agente y del negocio

- Canales: son distintos. Un WhatsApp por local (3); en uno se atienden consultas
  de venta online; aparte uno corporativo y otro de alquiler. Dos IG: venta y
  alquiler. ➜ **V1: solo alquiler, número nuevo.**
- Nombre y presentación ➜ **Lucía**: «Hola, soy Lucía, asistente de Mr. Otto.
  ¿En qué puedo ayudarte hoy?»
- Tono: cercano, sin tantos emojis.
- Siempre usar: *alquiler a medida*, *prendas de calidad*, *diseños nuevos*,
  *solución completa*. Nunca decir "NO".

## Catálogo, talles y stock (Mr Otto online) — fuera de la V1

- Catálogo y precios: sistema de gestión DUX. ➜ Sin integración en V1.
- Sin stock: prenda parecida o lista de espera.
- Guía de talles: depende de prenda y proveedor; la mayoría tiene tabla adjunta.
- Líneas: **Urban Concept** (uso diario y ocasiones especiales, elegant sport:
  camisas, pantalones, sacos, sweaters, camperas, bermudas). **Sastrería** (traje
  todos los días y ocasiones especiales; asesoramiento más personalizado y
  ajustes de sastrería). **Oportunidades** (productos seleccionados con precios
  promocionales, de las dos líneas).

## Alquiler de trajes (Otto Su Misura) — la V1

- El agente **puede gestionar la reserva** (agenda el turno). ➜ Escribe en base +
  Google Calendar; el panel muestra el calendario. Turnero actual:
  https://app3.doyturnos.com/ottoalquiler
- Qué incluye: el precio base incluye **solo el ambo/traje**. Aparte se alquilan
  camisa, cinturón, zapatos y accesorios según necesite. Quien alquila puede
  optar por prendas de venta con descuento (camisa, cinturón, zapatos, accesorios).
- Precio orientativo: el alquiler más básico **desde $150.000** (varía por modelo).
  ➜ Incluye sastrería y tintorería antes y después del evento.
- Accesorios (vigentes ➜ confirmado): camisa + corbata $33.500; zapato + cinto
  $55.000.
- Anticipación: recomendado entre 60 y 7 días antes. Se retira un día antes del
  evento (con prueba final) y se devuelve un día hábil después. En la práctica hay
  gente que alquila la misma semana o el mismo día y **nunca se le dice que no**.
- Talles: del XS al **68** de saco. Fuera de ese rango se puede confeccionar con
  tiempo, pero solo para la venta.
- Reserva y garantía: el alquiler se paga 100% al reservarlo. Tarjeta de crédito
  como garantía el día de la prueba final. Hay tabla de daños por grado ➜ **el
  agente no la comparte: deriva siempre.**
- Solo retiro en el local: España 764, Rosario. No se envía.
- Niños: hay trajes desde el talle 4.

## Sastrería a medida y confección (venta) — fuera de la V1

- Toma de medidas siempre presencial. Tiempos y mínimos: sin responder.

## Uniformes y corporativo — el agente deriva siempre

- Datos a pedir antes de derivar: cantidad de personas (hombres y mujeres), rubro,
  prendas actuales, si tienen logo, proveedor actual.
- Contacto separado para lo corporativo. Mínimos y precios por volumen: sin responder.

## Pagos, envíos y cambios (venta) — fuera de la V1

- 10% por transferencia y 3 cuotas sin interés: vigentes.
- Envío gratis en Rosario y desde $200.000 nacional: vigente.
- Rosario: en el día si el pedido es antes del mediodía, o al día siguiente. Resto
  del país según transporte.
- Cambios: https://www.mrotto.com.ar/cambios-y-devoluciones/

## Cuándo pasa a una persona

- Nunca resuelve solo: reclamos, turnos urgentes, prenda de alquiler dañada,
  pedido corporativo (todos).
- Cómo deriva: «le derivo tu inquietud a …», a números distintos según alquiler /
  venta / corporativo.
- Fuera del horario humano ➜ atiende y agenda igual, solo en horario laboral de
  Mr Otto. Si deriva: «Le paso tu consulta al equipo y te escriben apenas abran mañana.»

## Herramientas y contenido

- Número nuevo de WhatsApp. Hoy responden desde el teléfono directamente, sin
  Chatwoot ni Meta Business Suite.
- Fotos: de la web ➜ links cargados en las fichas del catálogo.
- Nunca compartir: costos internos, proveedores, precios sin consultar, tablas de
  talles no chequeadas.

## Lo que ya saben por experiencia

Preguntas más frecuentes (alquiler), tal cual:
- «precio del alquiler»
- «Necesito me pasen información y costos»
- «Cuanto sale el alquiler de un traje completo?»
- «Que incluye el alquiler?»
- Después preguntan por colores específicos.

Objeciones: sin responder ➜ los guiones los arma Mateo desde el ancla de valor y
el dueño los corrige en el panel.

Respuestas armadas: no reutilizar tal cual, «hoy informan, no asesoran ni venden».

## Ancla de valor (➜ Mateo, 12/9)

Mr Otto no alquila cualquier traje: se confecciona / ajusta a medida así queda
perfecto el día del evento. El precio-calidad-servicio es el mejor del mercado, y
eso lo diferencia de otros locales de alquiler. «Acá nos preocupamos de que tu
apariencia sea lo primero: el día de esa fecha especial es lo que más nos importa.»

## Agenda (➜ OTTO BOT, vigente)

- Lun–Vie 10:00–19:00; Sáb 9:30–18:30. Corte 14–15 (un probador puede cortar 13–14).
  ➜ **Todos cortan 14–15; la excepción 13–14 no aplica** (Mateo, 12/9).
- 3 probadores, escalonados de a 15 min.
- Graduado 45' · Novio 45' · Invitado 45' · Doble 1:30 (mismo probador) · Triple 2:00
  · Prueba final 15'.
- Un acompañante por persona. 10 min de tolerancia. Avisar si no puede asistir.
- Confirmación ➜ 24 hs antes por plantilla con botón (código puro).
- Post-devolución ➜ agradecer, pedir reseña de Google si le pareció buen servicio,
  pedir fotos del evento.
- Recontacto de presupuesto/consulta sin turno: al día siguiente y a las 72 hs.

## Enlaces

- Turnero actual: http://app3.doyturnos.com/ottoalquiler
- Mapa: https://maps.app.goo.gl/YKyU8qDRqDfNNvWT8
- Web Mr Otto: https://www.mrottocollection.com.ar/ · https://www.mrotto.com.ar/
- IG alquiler: @otto_sumisura · IG venta: @mrotto.ar
- WhatsApp alquiler (actual): 341 239-2502
