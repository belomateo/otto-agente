...
| 33 | Resuelto el 15/9 (Mateo, agente): a lo que no es texto (foto, audio, ubicación, sticker) Lucía le contesta con un texto fijo, editable desde Configuración › Lucía (`contexto_agente.texto_mensaje_no_soportado`), sin pasar por el modelo — antes no contestaba nada. El botón «Necesito reprogramar» sigue sin resolver: queda anotado en la bitácora, nada más | Worker, agente (H2.1) | Resuelto / agente (botón, pendiente) |
...
| 35 | El mail se pide una sola vez por charla, al ofrecer turnos, si la ficha no lo tiene. Si el cliente no lo quiere dar, se agenda igual y no se insiste. Un mail nuevo y válido reemplaza al anterior (el cliente lo corrigió) | 2.3, prompt y extractor | Mr Otto |
