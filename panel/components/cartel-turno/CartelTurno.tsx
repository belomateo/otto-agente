...
// falta + OK; con dos o más, "N turnos por avisar" + el más próximo, y tocarla abre una hoja
// con la lista (cada fila con su propio OK). Tocar cualquier fila (fuera del OK) abre el
// detalle completo en otra hoja — mismo patrón (fondo + panel) que ya usa MasSheet para
// "Más": así el detalle sí tapa todo mientras está abierto, pero nada lo hace mientras no se
// pidió.
//
...
function faltan(desde: string) {
  const minutos = Math.round((new Date(desde).getTime() - Date.now()) / 60_000);
...
function Detalle({ turno }: { turno: TurnoPorAvisar }) {
  return (
    <>
      <div className={`mt-0.5 ${CAMPO}`}>
        {hora(turno.desde)}–{hora(turno.hasta)} · {turno.tipo} · Probador {turno.probador}
      </div>
      <div className="my-2.5 flex flex-wrap items-center gap-1.5">
        <Chip estado={turno.confirmadoPor === 'cliente' ? 'Confirmado' : 'Sin confirmar'}>
          {turno.confirmadoPor === 'cliente' ? 'Confirmó por WhatsApp' : 'No confirmó todavía'}
        </Chip>
      </div>
      <div className="grid grid-cols-2 gap-x-3 gap-y-1">
        <Dato etiqueta="Teléfono" valor={turno.telefono} />
        <Dato etiqueta="Evento" valor={turno.evento ? `${turno.evento} · ${turno.fechaEvento}` : ''} />
        <Dato etiqueta="Rol" valor={turno.rol} />
        <Dato etiqueta="Talle" valor={turno.talle} />
        <Dato etiqueta="Color" valor={turno.color} />
      </div>
      {turno.notas && (
        <div className={`mt-1.5 ${CAMPO}`}>
          <span className={ETIQUETA}>Notas:</span> {turno.notas}
        </div>
      )}
    </>
  );
...
function Enlaces({ turno }: { turno: TurnoPorAvisar }) {
  return (
    <>
      <Link href={turno.charlaUrl} className="flex-1 rounded-otto border border-borde bg-lino py-2 text-center text-[14px] font-medium text-grafito md:text-[13px]">
        Ver charla
      </Link>
      <Link href={turno.fichaUrl} className="flex-1 rounded-otto border border-borde bg-lino py-2 text-center text-[14px] font-medium text-grafito md:text-[13px]">
        Ver ficha
      </Link>
    </>
  );
...
function TarjetaEscritorio({ turno, onOk }: { turno: TurnoPorAvisar; onOk: () => void }) {
  return (
    <div className="w-[380px] rounded-otto border border-cobre bg-lino p-4 shadow-otto-pop">
      <div className="flex items-baseline gap-2">
        <span className="min-w-0 flex-1 truncate font-serif text-lg font-semibold">{turno.cliente}</span>
        <span className="flex-none rounded-pill bg-cobre-claro px-2.5 py-[3px] text-[14px] font-medium text-cobre md:text-xs">{faltan(turno.desde)}</span>
      </div>
      <Detalle turno={turno} />
...
function FilaCelular({ turno, onAbrir, onOk }: { turno: TurnoPorAvisar; onAbrir: () => void; onOk: () => void }) {
  return (
    <div className="flex items-center gap-2 rounded-otto border border-cobre bg-lino py-2 pl-3.5 pr-2 shadow-otto-pop">
      <button type="button" onClick={onAbrir} className="flex min-w-0 flex-1 items-center gap-2 py-0.5 text-left">
        <span className="min-w-0 flex-1 truncate font-serif text-[15px] font-semibold">{turno.cliente}</span>
        <span className="flex-none rounded-pill bg-cobre-claro px-2 py-[2px] text-[14px] font-medium text-cobre">{faltan(turno.desde)}</span>
      </button>
      <button
        type="button"
        onClick={onOk}
        aria-label={`OK, turno de ${turno.cliente}`}
...
function FilaResumen({ cantidad, proximo, onAbrir }: { cantidad: number; proximo: TurnoPorAvisar; onAbrir: () => void }) {
  return (
    <button type="button" onClick={onAbrir} className="flex w-full items-center gap-2 rounded-otto border border-cobre bg-lino py-2 pl-3.5 pr-3 text-left shadow-otto-pop">
      ...
      <span className="min-w-0 flex-1 truncate rounded-pill bg-cobre-claro px-2 py-[2px] text-[14px] font-medium text-cobre">
        {proximo.cliente} · {faltan(proximo.desde)}
      </span>
    </button>
  );
}
...
function HojaLista({ turnos, onCerrar, onAbrirDetalle, onOk }: { turnos: TurnoPorAvisar[]; onCerrar: () => void; onAbrirDetalle: (id: string) => void; onOk: (id: string) => void }) {
  return (
...
function HojaDetalle({ turno, onCerrar, onOk }: { turno: TurnoPorAvisar; onCerrar: () => void; onOk: () => void }) {
  return (
    <div role="dialog" aria-label={`Turno de ${turno.cliente}`} className="fixed inset-0 z-[70] flex flex-col justify-end">
      <div className="flex-1 bg-tinta/[.32]" onClick={onCerrar} />
      <div className="rounded-t-2xl bg-lino px-[18px] pb-[26px] pt-5">
        <div className="mx-auto mb-3.5 h-1 w-9 rounded-pill bg-borde" />
        <div className="flex items-baseline gap-2">
          <span className="min-w-0 flex-1 truncate font-serif text-lg font-semibold">{turno.cliente}</span>
          <span className="flex-none rounded-pill bg-cobre-claro px-2.5 py-[3px] text-[14px] font-medium text-cobre">{faltan(turno.desde)}</span>
        </div>
        <Detalle turno={turno} />
...
export function CartelTurno() {
  ...
  const ordenados = [...turnos].sort((a, b) => new Date(a.desde).getTime() - new Date(b.desde).getTime());
