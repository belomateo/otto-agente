...
// Turnos del aviso (H1.16): se borran con los del cliente de prueba; su historial, antes.
const turnosAviso = [];
async function limpiar() {
  if (turnosAviso.length) {
    await q("delete from historial_ediciones where tabla = 'turnos' and fila_id = any($1::uuid[])", [turnosAviso]);
  }
...
async function verificarLimpieza() {
  ...
  if (turnosAviso.length) {
    ...
    ok(t.turnos === 0 && t.historial === 0, `no quedaron los turnos del aviso ni su historial (${t.turnos}, ${t.historial})`);
  }
...
try {
  ...
  seccion("H1.16 — aviso de turno antes de que empiece (decisión #10)");
  {
    ...
    turnosAviso.push(tA, tB, tC, tD, tE, tF);
    ...
    const nuestros = (g.datos.turnos ?? []).filter((t) => turnosAviso.includes(t.id));
    ok(
