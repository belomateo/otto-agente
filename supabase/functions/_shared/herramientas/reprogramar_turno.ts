...
// si el evento es hoy o mañana no se mueve nada: se deriva en código (decisión #8); el hueco
// nuevo es futuro, salió de buscar_horarios en este turno para el tipo del turno, entra en una
// franja de turnos en un probador que toma turnos ahí, dura lo que corresponde y no cae
// después del evento. Actualiza la misma fila (nunca crea otra encima), vuelve a "sin
// confirmar", mueve el evento de Calendar y manda la confirmación nueva armada en código.
...
export const reprogramarTurno: Herramienta<Args> = {
  ...
  async ejecutar(args, ctx) {
    ...
    if (esEventoInminente(ficha.fecha_evento, ctx.ahora, ctx.tz)) return await derivarPorEventoInminente(ctx);
    ...
    if (!enFranja.ok) {
      return rechazo("fuera_de_horario", `Ese horario queda fuera de los horarios de turnos: ${enFranja.motivo}. Ofrecé otro de buscar_horarios.`);
    }
    ...
    const movido = await moverAProbadorLibre(ctx.db, { turnoId: t.id, inicio, fin, probadores });
    if (!movido) {
